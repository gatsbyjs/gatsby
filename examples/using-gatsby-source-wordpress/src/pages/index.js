...
import createPermalinkPath from "gatsby-source-wordpress/utils/create-permalink-path"

import { Stack, Box, Heading, Text, Grid } from "@chakra-ui/core"
import Layout from "../components/layout"

export default ({ data }) => {
  const pages = [...data.allWpPost.nodes, ...data.allWpPage.nodes]
  ...
  return (
    <Layout>
      ...
      <Stack spacing={5}>
        {pages.map(page => (
          <Box key={page.path}>
            <Link to={createPermalinkPath(page.link)}>
              <Box p={5} shadow="md" borderWidth="1px">
                <Grid templateColumns="1fr 2fr" gap={6}>
                  <Box>
                    {!!page.featuredImage && (
                      <img alt="" src={page.featuredImage.sourceUrl} />
                    )}
                  </Box>
                  <Box>
