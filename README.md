...
- **Go Beyond Static Websites.** Get all the benefits of static websites with no limitations.
  Gatsby sites are fully functional React apps, so you can create high-quality, dynamic web apps, 
  from blogs to e-commerce sites to user dashboards.
...
- **Use a Modern Stack for Every Site.** No matter where the data comes from, Gatsby sites are
  ...
  whether the data is coming from the same backend or not.

- **Load Data From Anywhere.** Gatsby pulls in data from any data source, whether it is a Markdown
  file, a headless CMS like Contentful or WordPress, or a REST or GraphQL API. Use source plugins
