...
If the query is just a normal every-day query (not StaticQuery), then Gatsby updates its component's `query` in the Redux `components` namespace via the [replaceComponentQuery](https://github.com/hasthamalp/gatsby/blob/master/packages/gatsby/src/redux/actions/index.ts) action.
