...
> _Warning: gatsby-image is **not** a drop-in replacement for `<img />`. It’s optimized for fixed width/height images and images that stretch the full-width of a container._
...
Automatically create sizes for different resolutions — Gatsby creates responsive images for 1x, 1.5x, and 2x pixel densities using the `<picture>` element.
...
This is where fragments like `GatsbyImageSharpFluid` come in handy, as they'll return all the above items in one line:
