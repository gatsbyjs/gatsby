...
Keep in mind that in the example above the link of `posts` in `AuthorJson` works because `frontmatter` and `author` are both objects. But if for example, the `Frontmatter` type would have a list of `authors` instead (`frontmatter.authors.email`), it wouldn't work since the `by` argument doesn't support arrays. In that case, you'll have to provide a custom resolver with [Gatsby Type Builders](/docs/schema-customization/#gatsby-type-builders) or [createResolvers API](/docs/schema-customization/#createresolvers-api).
