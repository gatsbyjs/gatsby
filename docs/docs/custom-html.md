...
> Customizing `html.js` is a workaround solution for when the use of the appropriate APIs is not available in `gatsby-ssr.js`. Consider using [`onRenderBody`](/docs/reference/builds/gatsby-ssr/#onRenderBody) or [`onPreRenderHTML`](/docs/reference/builds/gatsby-ssr/#onPreRenderHTML) instead of the method above.
...
If you want to insert custom HTML into the footer, `html.js` is the preferred way of doing this. If you're writing a plugin, consider using the `setPostBodyComponents` prop in the [Gatsby SSR API](/docs/reference/builds/gatsby-ssr/).
