...
Note that because `useStaticQuery` is a React Hook, it is simply a function and so can be assigned to `data` which is then passed as a prop to the `PureHeader`. This is only possible since we have also made `Header` a function component.
