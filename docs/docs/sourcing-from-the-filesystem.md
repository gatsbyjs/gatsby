...
![Screenshot of the GraphiQL IDE showing the new autocomplete options provided by the `gatsby-source-filesystem` plugin](./images/graphiql-filesystem.png)

Hit <kbd>Enter</kbd> on `allFile` then type <kbd>Ctrl + Enter</kbd> to run a
query.

![Screenshot of the GraphiQL IDE showing the results of a filesystem query](./images/filesystem-query.png)

Delete the `id` from the query and bring up the autocomplete again (<kbd>Ctrl +
Space</kbd>).

![Screenshot of the GraphiQL IDE showing autocomplete options](./images/filesystem-autocomplete.png)
