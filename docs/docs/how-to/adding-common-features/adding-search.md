...
For other libraries, you can use a combination of [`onCreateNode`](/docs/reference/builds/gatsby-node/#onCreateNode), [`setFieldsOnGraphQLNodeType`](/docs/reference/builds/gatsby-node/#setFieldsOnGraphQLNodeType) and [`sourceNodes`](/docs/reference/builds/gatsby-node/#sourceNodes) from the Gatsby node API to create the search index and make it available in GraphQL. For more info on how to do this check out [`gatsby-plugin-elasticlunr-search`'s source code](https://github.com/gatsby-contrib/gatsby-plugin-elasticlunr-search/blob/master/src/gatsby-node.js#L96-L131).

Another option is to generate the search index at the end of the build using the [`onPostBuild`](/docs/reference/builds/gatsby-node/#onPostBuild) node API. This approach is used by [`gatsby-plugin-lunr`](https://github.com/humanseelabs/gatsby-plugin-lunr) to build a multilanguage index.
