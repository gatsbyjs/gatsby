...
any other theme so you end up with a tree of themes which you call a theme
composition.
...
These characteristics are what you use in the component shadowing algorithm
to decide which component to render. So, for example, if `gatsby-theme-tomato-blog`
has `gastby-theme-parent` as a parent theme you'd result in the following themes
array:
...
First, check for themes in the redux store. This is for
...
The `request` contains the path of the file which you want to use to find
...
module.exports = class GatsbyThemeComponentShadowingResolverPlugin {
  ...
  getMatchingThemesForPath(filepath) {
    // find out which theme's src/components dir you're requiring from
...
and `gatsby-theme-tomato` is installed on the site, you'll return `gatsby-theme-tomato`.

This match is performed so that we can determine the theme that is being required
from so you can check for shadowed files in the user's site or other themes to match
against. Also, make sure the matched themes are unique because two themes can
bring in the same theme to the theme composition. When that's the case you won't worry
...
Next, Check to make sure that if there is more than one matching theme there
is some sort of ambiguity and you should return an error. This can happen if there's
a path like `gatsby-theme-blog/src/components/gatsby-theme-something/src/components`
in the project.

#### No matches

If there are no theme matches you return the invoked callback because there's nothing
...
Now, if you still haven't returned the callback or thrown an error (due to ambiguity) it means you
have a file being required from a theme. The file being required will look something
like `/some/path/my-site/gatsby-theme-tomato/src/box` and so the first thing you
...
So, with the example path above you'll end up with `/box`. This can then be used to
see if the user's site or any other themes are shadowing the file.

### The component shadow

Since a file from a theme is being required you need to figure out which path should
be resolved for the requested component. You do this by calling `resolveComponentPath`
which uses the theming algorithm to attempt to find a shadowed component. If nothing
is found you return the original request which points at the original theme's
component.
...
You call `doResolve` on the resolver which specifies the shadowed component path if
...
When looking for a component you perform a search that occurs in two locations:
...
important to note again that the themes array is reversed. This is how you ensure that
"the last theme wins" when resolving a shadowed file. You walk the list for matches
...
behavior of webpack so you don't need to resolve a theme to itself.
  ...
  if (!this.cache[`${theme}-${component}`]) {
    this.cache[`${theme}-${component}`] = [
      ...
      .find(possibleComponentPath => {
        ...
        let dir
        try {
          // you use fs/path instead of require.resolve to work with
...
you want it to be possible to import the very component you're shadowing so that
...
This is the first case you'll handle when attempting to resolve the file.

In order to do this you need to leverage the **issuer** of the request. This points to
...
This is implemented by another method on the plugin's class which you call
...
Then, you join the component path and end up with:
...
You then know that if the issuer _matches_ one of these components that it's
being extended. This means that a shadowed component is extending the same component
from its parent.

When this happens, you return the next path, so here the original location of the theme:
`/Users/johno/c/gatsby-theme-example-component-extending/gatsby-theme-tomato/src/box`.

This means that when your shadowed file imports Box from a shadowed file you return the
original box component defined in the theme.

As a result, the following will work as you expect:
...
the issuer will be null. As such you need to first check that the `request.context.issuer`
is present before you attempt to resolve the shadowed component.
