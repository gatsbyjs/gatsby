...
These characteristics are what are used in the component shadowing algorithm
to decide which component to render. So, for example, if `gatsby-theme-tomato-blog`
has `gastby-theme-parent` as a parent theme it results in the following themes
array:
