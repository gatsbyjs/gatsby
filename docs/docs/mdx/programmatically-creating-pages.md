...
`gatsby-plugin-mdx` automatically adds a `slug` field to each MDX node derived from the corresponding file names. You can use special characters, whitespaces, or uppercase in the file names, and `gastby-plugin-mdx` will "slugify", making the slugs safe for serving over the internet and human readable. 
