...
To enable profiling for a Production build an additional CLI option `--profile` must be provided when running the build command. See [gatsby build command](/docs/reference/local-development/gatsby-cli/#options-1) for further information.
...
Capturing page performance can be achieved by using the [wrapPageElement API](/docs/reference/builds/gatsby-browser/#wrapPageElement) to profile each page.
