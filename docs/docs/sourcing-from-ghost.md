...
Once the source plugin is set up, you can use the [`createPages` API](/docs/reference/builds/gatsby-node/#createPages) in `gatsby-node.js` to run queries against your Ghost data with GraphQL. In this example, Gatsby iterates over each post returned by the Ghost API and generates a new page with that data, using the `post.js` template file.
