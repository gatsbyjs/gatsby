...
Once the initial bootstrap is finished `webpack-dev-server` is started, and an express server for serving files for the development server, and for a production build, is starting to build a CSS then JavaScript then HTML with webpack.
