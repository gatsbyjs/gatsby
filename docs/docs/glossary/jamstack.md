...
A software layer then pulls it all together. This layer retrieves content from the database, and replaces tags in the template files with the appropriate content pieces of data, and returns it to the browser. A single page gets regenerated each time the server receives a request for that URL.
...
A JAMStack backend consists of a content API that returns JSON or XML. Your API can be a [hosted datastore](https://www.gatsbyjs.org/docs/sourcing-from-hosted-services/), a [headless CMS](https://www.gatsbyjs.org/docs/headless-cms/), or a custom application. The same API can serve your Gatsby site and your mobile or desktop applications.
