...
The cache API is passed to [Gatsby's Node APIs](/docs/reference/builds/gatsby-node/) which is typically implemented by plugins.
...
The [Node API helpers](/docs/reference/builds/node-api-helpers/#cache) documentation offers more detailed information on the API.
...
Since cache files are stored within the `.cache` directory, deleting it will clear all cache. You can also use [`gatsby clean`](/docs/reference/local-development/gatsby-cli/#clean) to delete the `.cache` and `public` folders.
