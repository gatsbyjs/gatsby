...
The `StaticImage` component requires a few additional plugins to work. These extra plugins are called **peer dependencies**, and if your npm version is lower than 7 you'll need to install them along with the `gatsby-plugin-image` package:
...
**Tip:** Automatically installing peer dependencies is an exciting new feature introduced in npm 7. If your version of npm is 7 or higher then you won't need to install `gatsby-plugin-sharp` manually, npm will handle installing it automatically.
