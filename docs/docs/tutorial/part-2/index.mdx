...
The `Link` component lets you add a link to another page in your Gatsby site. You use it similarly to how you'd use an HTML `<a>` tag, but it gives you some extra performance benefits that the `<a>` tag doesn't have. The `Link` component takes in a `to` prop, which is the pathname to the page on your site you want to link to.
...
![A screenshot of the Home page. Now, under the heading, there's a link to the About page.](./index-with-link.png)
