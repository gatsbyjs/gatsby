...
**Note?**: Some of the vidoes like one below do not reflect the latest changes to the Tutorial.
We've update the code spippets in new commits to show some of the latest improvments to Gastby so the written content is more recent.
We'll do our best to record a new version in the future, thanks for understanding!
...
You can use the [Gatsby Head API](/docs/reference/built-in-components/gatsby-head/) by exporting a named function called `Head` in your pages and templates that are used to create pages. 

Be sure to capitalize `Head` and please note that exporting this named function inside a component won't add the metadata to the `<head>`. The above works because you're exporting `Head` in a page inside `src/pages`.
