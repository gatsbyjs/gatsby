...
The `Link` component lets you add a link to another page in your Gatsby site. You use it similarly to how you'd use an HTML `<a>` tag, but it gives you some extra performance benefits that the `<a>` tag doesn't have. The `Link` component takes in a prop called `to`, which is the pathname to the page on your site you want to link to.
...
Instead, it would be better to create one common `Layout` component that you can group all the shared elements to reuse across multiple pages. That way, when you need to make updates to the layout, you can make the change in one place and it will automatically be applied to all the pages that use that component.
...
In the next section, you'll learn about how to ues Gatsby plugins to add more pre-built functionality to your site.
