...
You'll need two information from Gatsby Cloud:
...
For example, the node above with the `text/markdown` MIME type will be transformer by `gatsby-transformer-remark` into further `MarkdownRemark` nodes.
...
export const sourceNodes = ({ parentSpan }) => {
  const tracer = opentracing.globalTracer()

  const fetchSpan = tracer.startSpan(`sourceNodes.fetch`, {
    childOf: parentSpan,
  })
