...
After copy & pasting the above contents of the index page into your file, your editor might complain about `Queries.IndexPageQuery` (TypeScript error). This type gets autogenerated through the GraphQL query at the bottom of the file. Your editor might need a second until it finds the type. If it doesn't fix itself, a reload of the code editor often helps.
