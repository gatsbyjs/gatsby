...
Gatsby always had best-in-class image support with its [`gatsby-plugin-image`](/plugins/gatsby-plugin-image/) feature. With it you can easily add responsive, optimized images to sites in all modern formats. Traditionally, images were downloaded and transformed to enable this behavior. Image CDN solves this by skipping image processing during the build, and instead deferring and offloading it to a dedicated image CDN. Visitors still get images in the most optimized formats such as AVIF and WebP, but site builds complete in a fraction of the time.
...
Gatsby's Image CDN feature can be added to any Gatsby source plugin. We'd recommend using it over manual file downloading for the reasons above. As long as your API gives back an URL to an image, you should be able to implement the required code. Basically, for Image CDN to work you need two things:
...
So for node types that are images this means: `publicUrl`, `mimeType`, `filename`, `width`, and `height` are mandatory.
...
You'll create a new root node called `PluginAsset` through a `createAssetNode` utility. You can think of it as a more specialized version of the `nodeBuilder` utility, only responsible for creating nodes that should become `PluginAsset` and hence Image CDN capable. Creating a separate `PluginAsset` type will also make schema customization in the next task easier.
...
1. Add the missing fields to the node:
    ...
    And that's it, you can create `PluginAsset` nodes now! In the next task you'll actually use `createAssetNode`.

### Task: Apply schema customizations

So far your `createAssetNode` utility isn't doing anything. You'll need to use it inside the `nodeBuilder` utility to add an additional `image` field to `Post` nodes. However, this will only work once you successfully implemented the second requirement of Image CDN: That `PluginAsset` implements the `RemoteFile` interface.
...
You'll implement the `RemoteFile` interface for `PluginAsset` and make it a root node. Then you'll create a foreign-key relationship between `PluginAsset` and the `image` field on `Post`.

1. Open `plugin/src/create-schema-customization.ts`. Change the name of `PostImage` to `PluginAsset` and implement both `Node` and `RemoteFile` for it:
...
1. Create a foreign-key relationship between `Post` and `PluginAsset` through `@link` on the `image` field.
...
1. A node of type `Post` should have the generated `id` of `PluginAsset` as its `image` field. Because only posts can have images. You can conditionally add data to the node if certain contiditions are met like this:
...
1. The `some-id` string should be replaced by the `PluginAsset` id of course. Since `createAssetNode` returns the `id`, you can use its result:
