...
1. Restart the `develop:site` script and open GraphiQL at `http://localhost:8000/___graphql`. Run the following query:
    ...
    It works! Note: `<long-string>` is added above to make things easier to read. If you're seeing an error, stop the `develop:site` script, run `pnpm run clean:site` and retry `pnpm run develop:site` again.
