...
1. A node of type `Post` should have the generated `id` of `Asset` as its `image` field. Because only posts can have images. You can conditionally add data to the node if certain contiditions are met like this:
