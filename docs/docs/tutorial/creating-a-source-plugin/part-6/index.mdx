...
**Pro tip:** `resize` and `gatsbyImage` will be provided by Gatsby. They are GraphQL resolvers relying on the existing data on the GraphQL node. You'll use `gatsbyImage` later to get your data for the `<GatsbyImage />` component.
