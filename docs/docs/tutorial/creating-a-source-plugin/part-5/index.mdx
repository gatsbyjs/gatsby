...
- Understand in more detail how delta updates work
...
1. Initialize a `isFirstSouce` boolean variable above the `sourceNodes` API. Inside `sourceNodes` you'll set it to false and then on subsequent runs the codepath it'll be used in won't get called again.
...
1. The `syncToken` is passed to the `fetchContent` utility function:
    ...
    And coming back is the `currentSyncData` or: The delta update.
