...
To pull data into a building-block component, you'll need to use a pre-defined function from Gatsby called `useStaticQuery`.
...
2. Inside your component, create a new variable and set it to the return value of `useStaticQuery`.
    * The `useStaticQuery` takes a single argument: a string (templated with the `graphql` tag) of your GraphQL query.
...
2. Paste the query from GraphiQL into useStaticQuery
...
3. Use the value from the response to render the title of your site in the JSX for your `Layout` component. Add it so it appears in both the browser tab and at the top of your page content.
...
4. Now that your data is showing up on the page, it's time to add some style! Define some styles for the site title below the existing styles in your `layout.module.css` file.
