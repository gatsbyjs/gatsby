...
2. Inside your component, call `useStaticQuery` using the `graphql` template tag and your query from GraphiQL. Store the results in a new variable so that you can use it later in your component.
    ...
    const data = useStaticQuery(graphql`
      ...
      // Change the name of the query to something unique (instead of the default "MyQuery")
    `)
    ```
...
2. Call `useStaticQuery` and pass it the query you created in GraphiQL. Be sure to use the `graphiql` tag so Gatsby knows that the string you're passing in is a GraphQL query. Store the return value from `useStaticQuery` in a variable.

<Announcement style={{marginBottom: "1.5rem"}}>

**Note:** Query names need to be unique, so update the default from `MyQuery` to something more descriptive.
...
Now that you have some posts saved to your local filesystem, it's time to pull those files into the Gatsby data layer. To do that, you'll use a special plugin called `gatsby-source-filesystem`.
...
![](./blog-page-with-post-filenames.png)
