...
- Render the MDX of each blog post.
...
Fortunately, we added the information we need to our frontmatter already. If you check the `.mdx` files we did create in the last part of this tutorial, you will see that their frontmatter has a `slug` field. If you run the following query:
...
**Note:** In this case, we use data we added to frontmatter to make the URLs for your blog posts easier for users to understand. If you don't want to enter this information for every blog post, the [readme of gatsby-plugin-mdx](https://github.com/gatsbyjs/gatsby/blob/master/packages/gatsby-plugin-mdx/README.md#slug) demonstrates how to programatically implement a `slug` field.
...
1. Create a new file in your `src/pages` directory called `{mdx.frontmatter_slug}.js`. This will be the file for your blog post page template.

2. Create a basic page component in your new `{mdx.frontmatter_slug}.js` file. For now, add in the `Layout` component, but hard code the page title and page contents. (You'll make those dynamic later on.)
...
```graphql
query MyQuery {
  mdx(frontmatter___slug: {eq: "another-post"}) {
    frontmatter {
      title
    }
  }
...
query MyQuery($slug: String) {
  mdx(frontmatter___slug: {eq: $slug}) {
    frontmatter {
      title
    }
  }
...
The last step of Part 6 is to clean up your Blog page. Instead of rendering a preview of your blog posts only, the Blog page should link out to the new post pages you just created.
