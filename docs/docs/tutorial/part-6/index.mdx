...
HOW COLLECTION ROUTES WORK. SOMETHING ABOUT PROPS THAT GET ADDED TO THE COMPONENT.

EXAMPLE OF QUERY/RESULT. LIST THE ROUTES THAT WOULD BE CREATED FOR THAT DATA.

</Collapsible>

In this part of the Tutorial, you'll use Gatsby's file system route API to dynamically create new pages for each of your blog posts. THE diagram BELOW SHOWS SOMETHING.
...
COLLECTION ROUTES. CREATE A FILE WITH THE `src/pages/{mdx.slug}` NAME. IT HAS TO BE IN SRC/PAGES. THIS CREATES THE NEW PAGES, USING THE SLUG AS THE URL PATH.
LOOK AT GRAPHIQL. GET EACH POST'S SLUG.

USE THE SLUG BECAUSE IT'S MORE HUMAN-READABLE THAN THE ID. BUT COULD USE THE ID IF NAMES AREN'T GUARANTEED TO BE UNIQUE, LIKE FOR PRODUCTS.
...
Go to `localhost:8000/my-first-post` and you should see your content. Can check again at the other slugs. (Get slugs from the file names or from GraphiQL.)

<Announcement style={{marginBottom: "1.5rem"}}>

YOU CAN SEE ALL THE PAGES CREATED ON THE 404 PAGE WHEN YOU RUN THE DEVELOP SERVER. GO TO A FAKE URL, AND THERE WILL BE A LIST OF PAGES CREATED FOR YOUR SITE.

</Announcement>

PRO TIP: IF YOU WANT TO HAVE MORE CONTROL OVER THE DIFFERENT NODES, TO ADD EXTRA FIELDS OR WHATEVER, YOU CAN USE THE GATSBY-NODE API. BUT THAT'S OUT OF SCOPE FOR THIS TUTORIAL. (MAYBE LEAVE THIS AS A SUGGESTED NEXT STEP IN THE "WHAT'S NEXT" SECTION.)

### TASK: UPDATE PATH TO INCLUDE BLOG URL PATH
