...
You can catch the stream live on Wednesdays at 10AM Pacific Time on the [Gatsby Twitch channel](https://www.twitch.tv/gatsbyjs).
...
**Having trouble with `gatsby new`?** If you had trouble globally installing Gatsby in Part 0, you can also create a new site by running `npm init gatsby` from the command line instead of `gatsby new`.
