...
To illustrate this, we'll walk through a small example site that uses both Gatsby's data layer, and also fetches data on the client. This example is based loosely on Jason Lengstorf's [Gatsby with Apollo](https://github.com/jlengstorf/gatsby-with-apollo) example. We'll be fetching character data for Rick (of Rick and Morty) and a random pupper image.
