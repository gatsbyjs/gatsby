...
In September we posted our [Series A funding announcement](https://gatsbyjs.org/blog/2019-09-26-announcing-gatsby-15m-series-a-funding-round), then headed to the UK for Gatsby Days, London. Fear not, if you missed all the action you can [catch up by watching the videos](https://youtu.be/UvO7h_KwgpE?list=PLCU2qJekvcN2f9rgS50xy7-4QBpgwid4G).
...
Following on from [an RFC](https://github.com/gatsbyjs/rfcs/pull/42) in September, there are now 10 in-progress translations for Gatsby's documentation. Want to get involved? Check out [the new Gatsby Docs Translation Guide](https://gatsbyjs.org/contributing/gatsby-docs-translation-guide). Thanks to [Nat Alison](https://twitter.com/tesseralis) for driving this project!

### The Gatsby Build Process

Have you ever wondered what Gatsby _does_ when you run a build? There's now a new docs page that walks through the details. [Overview of the Gatsby Build Process](https://gatsbyjs.org/docs/overview-of-the-gatsby-build-process) is the place to go. I particularly like [the diagram that explains the different layers of Gatsby](https://gatsbyjs.org/docs/overview-of-the-gatsby-build-process/#what-happens-when-you-run-gatsby-build).

### Schema Customization Examples

We're gradually moving our official plugins over to use the Schema Customization APIs we added earlier this year. See [the changes to `gatsby-transformer-sharp` for an example](https://github.com/gatsbyjs/gatsby/pull/18871). If you're interested in helping out with this, there's now a little known plugin that will write out the inferred schema to disk. See [gatsby-plugin-schema-snapshot](https://gatsbyjs.org/packages/gatsby-plugin-schema-snapshot).

### gatsby-plugin-offline

[gatsby-plugin-offline](https://gatsbyjs.org/packages/gatsby-plugin-offline) has been updated to [handle more edge cases](https://github.com/gatsbyjs/gatsby/pull/17590). The plugin will now revert back to the network if a visitor disables JavaScript after the site has already been cached.
...
Add the theme, add your Firebase credentials and off you go. Want to change anything? Use [component shadowing](https://gatsbyjs.org/blog/2019-04-29-component-shadowing) to override whatever you like.

## Gatsby Cloud

Last but _certainly_ not the least, we launched [Gatsby Cloud](https://www.gatsbyjs.com/cloud) last week! 💃

Here's an excerpt from [Kyle's announcement post](https://gatsbyjs.org/blog/2019-11-14-announcing-gatsby-cloud):
