...
![Tutorial](/content/images/2018/01/Screen-Shot-2018-01-17-at-21.29.59.png)
...
When your project is achieved, you will probably want to deploy it. The static website generated by Gatsby can [easily be published on storage providers](https://www.gatsbyjs.org/docs/deploy-gatsby/): Netlify, S3/Cloudfront, GitHub pages, GitLab pages, Heroku, etc. Since it is a Progressive Web App without server-side rendering, we strongly advise you to use a pre-rendering solution, such as Prerender.io. The Strapi API is nothing else than a simple Node.js API, so it can be hosted on Heroku or any Linux instance that has Node.js installed.
