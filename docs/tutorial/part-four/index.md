...
Notice `<StaticQuery/>` wrapping the component? `<StaticQuery/>` is utilized to query GraphQL data inside a component. It's usage can be clearly seen in `layout.js` above.
