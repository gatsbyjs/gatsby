...
Installs `gatsby-source-wordpress` to pull the WordPress data into Gatsby. Note, once this recipe finishes, you will need to update the config to point to your actual WordPress URL.
...
Creates a local Gatsby plugin that create pages for the WordPress data.
...
Creates basic post, page, category, tag, and user templates.
...
After installing this recipe, update `gatsby-config.js` to point to your WordPress instance. Then start Gatsby and check out your new pages powered by WordPress!
