  ...
  if (
    ...
    if (id.isObjectPattern()) {
      return id
        ...
        .find(
          (path) =>
            (path.get("value") as NodePath<Identifier>).node.name === name,
        ) as NodePath;
    }
    return id;
  }
...
function isGraphqlTag(tag: NodePath, tagName: string = "graphql"): boolean {
  ...
  if (importPath.isObjectProperty())
    return (
      (importPath.get("key") as NodePath<Identifier>).node.name === tagName
    );
      ...
      Program(path: NodePath<Program>, state: any): void {
        ...
        path.traverse({
          TaggedTemplateExpression(path2: NodePath<TaggedTemplateExpression>) {
            ...
            const potentialExportPath =
              path2.parentPath?.parentPath?.parentPath;
            if (
