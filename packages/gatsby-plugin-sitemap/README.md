...
- `excludes` (string[] = []) An array of paths to exclude from the sitemap. [Here are some examples](https://codepen.io/checkmark/pen/LYmOewV) of glob matching using [minimatch](https://github.com/isaacs/minimatch). While `excludes` is usually an array of strings it is possible to enter other data types into this array for custom filtering, but doing so will require customization of the [`filterPages`](#filterPages) function.
