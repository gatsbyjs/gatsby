...
function traversePackagesDeps({
  ...
  packages.forEach(p => {
    ...
    try {
      ...
      if (packageRoot) {
        pkgJson = require(_nodePath.default.join(packageRoot, `package.json`));
      } else {
    ...
    const fromMonoRepo = (0, _intersection2.default)(Object.keys({
    ...
    const newPackages = (0, _difference2.default)(fromMonoRepo, seenPackages);
