...
const mapUserLinkHeaders = ({ manifest, pathPrefix, publicFolder }) => (
  headers,
) =>
...
const mapUserLinkAllPageHeaders = (pluginData, { allPageHeaders }) => (
  headers,
) => {
  if (!allPageHeaders) {
    return headers
...
const applyCachingHeaders = (pluginData, { mergeCachingHeaders }) => (
  headers,
) => {
