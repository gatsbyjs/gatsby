...
Netlify will only upload assets to its CDN that have changed between deployments. That is to say that if deployment `A` is the initial upload of the whole site, then deployment `B` only includes a changed `404.html` file, only that file will be uploaded to the Netlify CDN.

By default all `.js` & `.css` assets generated by Gatsby will include content hashes in the filenames meaning if they change between deployments they will be uploaded. By itself this is not a problem, however these assets may be referenced in many `.html` files causing a lot of assets to be uploaded.

There is an option to turn off the behaviour in Gatsby and allow Netlify to handle cache invalidation using `etags`. To turn off content hashes for all `.js` & `.css` assets add:
