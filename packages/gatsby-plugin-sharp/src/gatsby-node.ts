...
import { setPluginOptions } from "./plugin-options";
...
function removeCachedValue(cache: GatsbyCache, key: string): Promise<void> {
  if (cache?.del) {
    ...
    // @ts-ignore Property 'cache' does not exist on type 'GatsbyCache'.ts(2339)
...
function splitOperationsByRequestedFile(
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  job: any,
  requestedPathOnDisk: string,
): {
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  matchingJob: any;
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  jobWithRemainingOperations: any;
} {
  const matchingJob = {
    ...job,
