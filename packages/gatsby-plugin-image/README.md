...
- **formats**: an array of file formats to generate. The default is `[AUTO, WEBP]`, which means it will generate images in the same format as the source image, as well as in the next-generation [WebP](https://developers.google.com/speed/webp) format. We strongly recommend you do not change this option, as doing so will affect performance scores. You may also add `AVIF`, which is a next-generation image format with better compression than either WebP or JPG, though browser support is currently limited.
