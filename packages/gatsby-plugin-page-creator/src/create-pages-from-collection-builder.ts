...
import { pluginInstances } from "./create-pages-from-changed-nodes"
...
${errors.map(error => error.message).join(`\n`)}`.trim(),
  ...
  if (trackedTypes) {
    console.log(`Setting tracked types 1`, trackedTypes)
    trackedTypes.set(nodeType, { nodeType, absolutePath })
  } else {
    console.trace(`Setting tracked types 2`, trackedTypes)
