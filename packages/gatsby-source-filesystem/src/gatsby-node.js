  ...
  return new Promise((resolve, reject) => {
    ...
    recursive(
      ...
      (err, files) => {
        ...
        mapLimit(mappedFiles, 15, (file, cb) => md5File(file.absolutePath, cb), (e, hashes) => {
          hashes.forEach((hash, index) => { mappedFiles[index].hash = hash })
          console.timeEnd(`hash files`)

          console.time(`stat files`)
          mapLimit(mappedFiles, 15, (file, cb) => fs.stat(file.absolutePath, cb), (er, stats) => {
            ...
            mappedFiles.forEach((file) => {
              ast.children.push({
                ...
                children: [],
                relativePath: slash(path.posix.relative(pluginOptions.path, file.absolutePath)),
