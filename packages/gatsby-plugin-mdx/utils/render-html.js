...
let findAsset = function (src, compilation, webpackStatsJson) {
  if (!src) {
...
class MdxHtmlBuilderWebpackPlugin {
  apply(compiler) {
    ...
    let afterEmit = (compilation, callback) => {
      ...
      try {
        let asset = findAsset(self.entry, compilation, webpackStatsJson)
    ...
    if (compiler.hooks) {
      let plugin = { name: `MdxHtmlBuilderWebpackPlugin` }
