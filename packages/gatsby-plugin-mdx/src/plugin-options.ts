...
export type IMdxPluginOptions = {
  extensions: [string]
  mdxOptions: ProcessorOptions
  gatsbyRemarkPlugins?: [IPluginInfo] | undefined
}
type IHelpers = {
...
export const defaultOptions: MdxDefaultOptions = function defaultOptions(
...
export const enhanceMdxOptions: EnhanceMdxOptions =
  async function enhanceMdxOptions(
    pluginOptions: PluginOptions,
    helpers: IHelpers,
  ): Promise<ProcessorOptions> {
    const options = defaultOptions(pluginOptions)

    const { default: remarkUnwrapImages } =
      await cachedImport<typeof import("remark-unwrap-images")>(
