...
import { getRepositoryId as _getRepositoryId, GitRepoMetaData } from './repository-id'
...
const GATSBY_TELEMETRY_TAGS: string | void = process.env.GATSBY_TELEMETRY_TAGS;

const defaultOpts = { debounce: false }
...
export default class AnalyticsTracker {
  ...
  gatsbyCliVersion: string | void
  installedGatsbyVersion: string | void
  repositoryId: GitRepoMetaData | void
  machineId: string | void = undefined
  ...
  getSessionId(): string {
    // TODO: gatsbyTelemetrySessionId is not defined on process
    // @ts-ignore
    return (process.gatsbyTelemetrySessionId || (process.gatsbyTelemetrySessionId = uuidV4()))
  }

  getRepositoryId(): GitRepoMetaData {
  ...
  getTagsFromEnv(): {} {
    if (GATSBY_TELEMETRY_TAGS) {
      try {
        return JSON.parse(GATSBY_TELEMETRY_TAGS)
      } catch (_) {
        // ignore
      }
    }
  ...
  getGatsbyCliVersion(): string | void {
    try {
      const jsonfile: string = join(
        require
  ...
  captureEvent(type = '', tags = {}, opts = defaultOpts): void {
  ...
  formatErrorAndStoreEvent(eventType: string, tags: Tag): void {
    if (tags.error) {
  ...
  buildAndStoreEvent(eventType: string, tags: {}): void {
    const event = {
