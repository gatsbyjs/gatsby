...
function buildLocalCommands(cli: Argv, isLocalSite: boolean): void {
  ...
  function getCommandHandler(
    command: string,
    handler?:
      | ((args: Arguments, cmd: (args: Arguments) => void) => void)
      | undefined,
    nodeEnv?: string | undefined,
  ): (argv: Arguments) => void {
    return (argv: Arguments): void => {
    ...
    builder: (_) => {
      return _.option("H", {
        ...
        .option("S", {
          ...
          type: "boolean",
          describe:
            "Use HTTPS. See https://www.gatsbyjs.com/docs/local-https/ as a guide",
        })
        .option("c", {
          alias: "cert-file",
          type: "string",
          default: "",
          describe:
            "Custom HTTPS cert file (also required: --https, --key-file). See https://www.gatsbyjs.com/docs/local-https/",
        })
        .option("k", {
          alias: "key-file",
          type: "string",
          default: "",
          describe:
            "Custom HTTPS key file (also required: --https, --cert-file). See https://www.gatsbyjs.com/docs/local-https/",
        })
        .option("ca-file", {
          type: "string",
          default: "",
          describe:
            "Custom HTTPS CA certificate file (also required: --https, --cert-file, --key-file).  See https://www.gatsbyjs.com/docs/local-https/",
        })
        ...
        .option("open-tracing-config-file", {
          type: "string",
          describe:
            "Tracer configuration file (OpenTracing compatible). See https://gatsby.dev/tracing",
        })
        .option("inspect", {
          type: "number",
          describe:
            "Opens a port for debugging. See https://www.gatsbyjs.com/docs/debugging-the-build-process/",
        })
        .option("inspect-brk", {
          type: "number",
          describe:
            "Opens a port for debugging. Will block until debugger is attached. See https://www.gatsbyjs.com/docs/debugging-the-build-process/",
        });
    },
    handler: handlerP(
      getCommandHandler(
        "develop",
        (args: Arguments, cmd: (args: Arguments) => unknown) => {
    ...
    builder: (_) => {
      return (
        _.option("prefix-paths", {
          ...
          describe:
            "Build site with link paths prefixed with the pathPrefix value in gatsby-config.js. Default is env.PREFIX_PATHS or false.",
        })
          .option("no-uglify", {
            type: "boolean",
            default: false,
          ...
          .option("profile", {
            ...
            describe:
              "Build site with react profiling (this can add some additional overhead). See https://reactjs.org/docs/profiler",
          })
          .option("graphql-tracing", {
            type: "boolean",
            describe:
          ...
          .option("functions-platform", {
            type: "string",
            describe:
              "The platform bundled functions will execute on. Defaults to current platform or settings provided by used adapter.",
          })
          .option("functions-arch", {
            type: "string",
            describe:
              "The architecture bundled functions will execute on. Defaults to current architecture or settings provided by used adapter.",
          })
      );
    },
    handler: handlerP(
      getCommandHandler(
        "build",
        (args: Arguments, cmd: (args: Arguments) => void) => cmd(args),
        "production",
      ),
    ),
  });
      ...
      return _.option("H", {
        ...
        .option("prefix-paths", {
          ...
          default:
            ...
            process.env.PREFIX_PATHS === "1",
          describe:
            "Serve site with link paths prefixed with the pathPrefix value in gatsby-config.js.Default is env.PREFIX_PATHS or false.",
        })
        .option("open-tracing-config-file", {
          type: "string",
          describe:
...
export function createCli(argv: Array<string>):
  | {
      [x: string]: unknown;
      ...
      builder: (yargs) => {
        return yargs
          ...
          .positional("value", {
            choices: ["npm", "pnpm"],
            type: "string",
            describe: "Set package manager as `npm` or `pnpm`.",
          });
      },
      handler: handlerP(({ cmd, key, value }) => {
        ...
        if (cmd === "set") {
          if (key === "pm" || key === "package-manager") {
            if (value && value !== "npm" && value !== "pnpm") {
              report.panic("Package manager must be npm or pnpm.");
            }
