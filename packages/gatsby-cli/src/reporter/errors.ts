...
function sanitizeStructuredStackTrace(stack: IStack): IStack {
  // first filter out not useful call sites
  stack = stack.filter(function filter(callSite: ICallSite): boolean {
  ...
  stack = stack.map(function callbackfn(callSite) {
    return pick(callSite, [`fileName`, `functionName`, `columnNumber`, `lineNumber`])
  })
