...
By default, the following query returns all headings in a markdown file. To get only a specific level of heading, specify the `depth` argument with a value `h[1-6]`. Moreover, the default value for the slug is an anchor determined by the heading's value, e.g. for a heading "1. Section one" it will be "/#1-section-one". This is the same anchor generated by [`gatsby-remark-autolink-headers`](https://www.gatsbyjs.org/packages/gatsby-remark-autolink-headers). To prepend any property of the markdown node, to this anchor, you can specify the `pathToSlugField`. Say you have a post with a slug property of "my-post" in its frontmatter, specifying `pathToSlugField: "frontmatter.slug"` will result in the slug "/my-post/#1-section-one". Lastly, you can specify a string such as "/blog" to prepend in front of that and get "/blog/my-post/#1-section-one" by providing the `prefix` argument.
...
```graphql
{
  allMarkdownRemark {
    edges {
      node {
        headings(
          # assumes you have a slug field in your frontmatter for all items returned by this query
