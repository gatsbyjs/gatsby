...
describe("gatsby-node", () => {
  ...
  it("filters content type with contentTypeFilter", async () => {
    ...
    await simulateGatsbyBuild({
      ...defaultPluginOptions,
      contentTypeFilter: (contentTypeItem) =>
        contentTypeItem.sys.id !== "person",
    });
  ...
  it("panics when response contains restricted content types", async () => {
    ...
    expect(reporter.panic).toBeCalledWith(
      expect.objectContaining({
        context: {
          sourceMessage:
            'Restricted ContentType name found. The name "reference" is not allowed.',
        },
      }),
    );
  });
  ...
  it("panics when response contains content type Tag while enableTags is true", async () => {
    ...
    expect(reporter.panic).toBeCalledWith(
      expect.objectContaining({
        context: {
          sourceMessage:
            'Restricted ContentType name found. The name "tag" is not allowed.',
        },
      }),
    );
  });
