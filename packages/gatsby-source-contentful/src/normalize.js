...
function hasExternalReference(fieldValue = {}) {
  return fieldValue.sys && fieldValue.sys.type && fieldValue.sys.id
}

function addAndMutateReferences({
  existingReferences = {},
  contentTypeItemId,
  reference,
}) {
  const contentTypeReferences =
    existingReferences[`${contentTypeItemId}___NODE`] || []
  contentTypeReferences.push(reference)

  existingReferences[`${contentTypeItemId}___NODE`] = contentTypeReferences
      ...
      Object.keys(entryItemFields).forEach(entryItemFieldKey => {
        if (entryItemFields[entryItemFieldKey]) {
          ...
          if (Array.isArray(entryItemFieldValue)) {
            if (hasExternalReference(entryItemFieldValue[0])) {
              entryItemFields[`${entryItemFieldKey}___NODE`] = []
              entryItemFieldValue.forEach(v => {
                const nodeId = mId(space.sys.id, v.sys.id)
                const contentTypeItemIdLowerCase = contentTypeItemId.toLowerCase()
                entryItemFields[`${entryItemFieldKey}___NODE`].push(nodeId)

                const references = addAndMutateReferences({
                  existingReferences: foreignReferenceMap.get(nodeId),
                  contentTypeItemId: contentTypeItemIdLowerCase,
                  reference: mId(space.sys.id, entryItem.sys.id),
                })

                foreignReferenceMap.set(nodeId, references)
              })
            ...
            if (
              ...
              const nodeId = mId(space.sys.id, entryItemFieldValue.sys.id)
              const contentTypeItemIdLowerCase = contentTypeItemId.toLowerCase()
              entryItemFields[`${entryItemFieldKey}___NODE`] = nodeId
