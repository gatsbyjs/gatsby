...
function preset(_context, options = {}) {
  ...
  return {
    presets: [[r(`@babel/preset-env`), Object.assign({
      loose: true,
      debug: !!debug,
    ...
    r(`@babel/plugin-transform-runtime`), r(`@babel/plugin-syntax-dynamic-import`), IS_TEST && r(`babel-plugin-dynamic-import-node`), [r(`./babel-transform-compiler-flags`), {
      flags: parsedCompilerOptions,
      availableFlags: availableCompilerFlags
    }], r(`@sigmacomputing/babel-plugin-lodash`), Array.isArray(keepDynamicImports) && keepDynamicImports.length > 0 && [r(`./babel-transform-mark-to-keep-dynamic-import`), {
      keepDynamicImports
    }]].filter(Boolean),
    overrides: [{
      test: [`**/*.ts`, `**/*.tsx`],
