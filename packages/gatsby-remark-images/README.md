...
By default, the text `Alt text here` will be used as the alt attribute of the generated `img` tag and for the contents of the generated `figcaption` element. If an empty alt attribute like `alt=""` and no figcaption is wished, a reserved keyword `GATSBY_EMPTY_ALT` can be used.
