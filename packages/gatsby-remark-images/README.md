...
| `fallbackToAltTextForCaptions` | `true` | If `showCaptions` is set to `true` and the title attribute is empty, this specifies whether the alt attribute should be used as a caption. If true it will be used, and if false the caption will be omitted if there is no title attribute. |
