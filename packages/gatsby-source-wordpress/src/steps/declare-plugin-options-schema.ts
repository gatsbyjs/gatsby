    ...
    type: Joi.object({
      ...
      MediaItem: Joi.object({
        lazyNodes: Joi.boolean()
          .default(false)
          .description(
            `Enables a different media item sourcing strategy. Instead of fetching Media Items that are referenced by other nodes, Media Items will be fetched in connection resolvers from other nodes. This may be desirable if you're not using all the connected images in your WP instance. This is not currently recommended because it messes up cli output and can be slow due to query running concurrency.`
          )
          .meta({
            example: wrapOptions(`
                type: {
