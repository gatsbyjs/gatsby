...
async function inlineImagesToGatsby(
  element,
  nodeOptions,
  pluginOptions,
) {
  const { node, actions, getNode, getNodes, store, cache, createNodeId } = nodeOptions;
  ...
  try {
    ...
    try {
      ...
      const fetchUrl = protocolInUrl ? element.properties.src : `${protocol}:${element.properties.src}`
      ...
      if (verboseOutput) {
        console.log(`Error creating node for: ${element.properties.src} -- Message: ${e}`);
      }
      return element
    }
  }
  if (fileNode) {
    ...
    if (verboseOutput) {
      console.log(`Error processing: ${element.properties.src} -- Message: No file node found or created`);
    }
  }
  return element
}
 ...
 * ]
 */
async function runHastTransformations(
  nodeOptions,
  pluginOptions,
) {
  ...
  for (const option of hastOptions) {
    if (option.nodeTypes.includes(node.internal.type)) {
      ...
      await Promise.all(targetElements.map(async (element) => {
        return option.transformFunction(element, nodeOptions, pluginOptions)
      }));
