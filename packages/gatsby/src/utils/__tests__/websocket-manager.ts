...
function waitUntil<T = any>(
  cb: (resolve: (resolved: T | PromiseLike<T>) => void) => void,
  ...
  return new Promise((resolve) => {
    ...
    const interval = setInterval(() => {
      ...
      cb((resolved: T | PromiseLike<T>): void => {
        clearInterval(interval);
        resolve(resolved);
      });
...
describe("websocket-manager", () => {
  ...
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  function getClientSocket(): io.Socket<any, any> {
    return io.default(`http://127.0.0.1:${httpServerAddr.port}`);
  }

  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  function getClientSocketAndWaitForConnect(): Promise<io.Socket<any, any>> {
    return new Promise((resolve) => {
  ...
  async function disconnectClientSocket(
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    clientSocket: io.Socket<any, any>,
  ): Promise<void> {
