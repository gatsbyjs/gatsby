...
const funcObj = {
  singleCall(str: string, num: number) {
    return `${str}, ${num}`
  },

  bulkCall(calls: Parameters<typeof funcObj.singleCall>[]) {
    return calls.length
  }
}
...
describe(`Batcher`, () =>  {
  ...
  function createBatcherWithSpies(threshold: number) {
  ...
  [1, 2, 10].forEach(threshold => {
    ...
    it(`flushes bulk calls after threshold of ${threshold} is hit`, () => {
      createBatcherWithSpies(threshold)
      const expectedCall = Array.from(Array(threshold).keys()).map((i) => ['string', i])
