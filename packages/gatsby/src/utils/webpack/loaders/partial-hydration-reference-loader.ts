        ...
        for (const specifier of node.specifiers) {
          if (
            ...
            if (specifier.exported.name === `default`) {
              references.push(
                createDefaultReference(specifier.local.name, moduleId)
              )
            } else {
              references.push(
      ...
      switch (node.declaration.type) {
        ...
        case `Identifier`:
          if (node.declaration.name) {
            references.push(
              createDefaultReference(node.declaration.name, moduleId)
            )
          }
          break
        case `FunctionDeclaration`:
        case `ClassDeclaration`:
          ...
          if (
            ...
            references.push(createDefaultReference(``, moduleId))
          }
          break
      }
    },
