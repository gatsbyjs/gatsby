...
import type { AdapterInit, IAdapter, IAdapterManifestEntry } from "../types";
...
let mockLogs: Array<{
  ...
  args: Array<any>;
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  spinnerArgs?: Array<any> | undefined;
}> = [];

function getLogsForSnapshot(): string {
  return mockLogs
    .map((log) => {
      return `${log.level.padEnd(13)} ${[
        ...log.args,
        ...(log.spinnerArgs ?? []),
      ]
...
jest.mock("gatsby-core-utils/create-require-from-path", () => {
  return {
    createRequireFromPath: jest.fn((path: string) => {
      ...
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      function siteRequire(mod: string):
        | AdapterInit
        | {
            version: string;
          } {
...
describe("getAdapterInit", () => {
  ...
  describe("matching adapter module for current environment", () => {
    ...
    it("panics if automatic installation of correct version of adapter fails", async () => {
      ...
      expect(
        mockLogs.find((log) => log.level === "spinner-panic"),
      ).toBeTruthy();
      expect(execa).toMatchInlineSnapshot(`
        [MockFunction] {
          "calls": Array [
