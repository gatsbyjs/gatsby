...
/**
 * To be compatible with Bluebirds mapSeries implementation we have to wait for all promises to be settleds
 * that are not awaited. Crazy right? Sadly plugins and our own implementations do rely on this behaviour (Yes, this is definitly a code smell).
 *
 * @param {() => unknown)} cb
 * @returns {Promise<unknown>}
 */
function runUntilAllUnawaitedPromisesAreSettled(cb) {
  const deferred = {
    promise: null,
    resolve: null,
    reject: null,
  ...
  const promiseQueue = new Set()
  const promiseListener = createHook({
    init(asyncId, type, triggerAsyncId) {
      if (type === `PROMISE` && promiseQueue.has(triggerAsyncId)) {
        promiseQueue.add(asyncId)
      }
    },
    promiseResolve(asyncId) {
      if (promiseQueue.has(asyncId)) {
        promiseQueue.delete(asyncId)
  ...
  promiseQueue.clear()
  promiseListener.enable()
  return Promise.resolve()
    .then(() => {
      promiseQueue.add(executionAsyncId())
      return cb()
    })
    .then(result => {
      promiseListener.disable()
...
function apiRunnerNode(api, args = {}, { pluginSource, activity } = {}) {
  ...
  return new Promise(resolve => {
    ...
    if (
      ...
      const promiseSerial = async (plugins, cb) => {
        const results = []
        for (const plugin of plugins) {
          results.push(
            await runUntilAllUnawaitedPromisesAreSettled(() => cb(plugin))
          )
        }

        return results
      }
      runPromise = promiseSerial
