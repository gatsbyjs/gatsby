...
function doubleBind(boundActionCreators: Record<string, string>, api, plugin, actionOptions) {
...
function initAPICallTracing(parentSpan?: opentracing.Span | opentracing.SpanContext | undefined): {
  tracer: opentracing.Tracer;
  parentSpan: opentracing.Span | opentracing.SpanContext | undefined;
  startSpan: (spanName: string, spanArgs?: opentracing.SpanOptions) => opentracing.Span;
} {
  const startSpan = (spanName: string, spanArgs: opentracing.SpanOptions = {}): opentracing.Span => {
...
async function runAPI(plugin, api, args, activity): globalThis.Promise<any> {
  ...
  if (gatsbyNode[api]) {
    ...
    const endInProgressActivitiesCreatedByThisRun = (): void => {
...
export function apiRunnerNode(
  ...
  } = {},
  {
    pluginSource,
    activity,
  }: { pluginSource?: string | undefined; activity?: any } = {}
