...
function generateQueryName({ def, hash, file, queryType }): any {
...
// there
// eslint-disable-next-line @typescript-eslint/no-explicit-any
function followVariableDeclarations(binding): any {
...
function referencesGatsby(path, callee, calleeName): boolean {
...
// eslint-disable-next-line @typescript-eslint/no-explicit-any
function warnForUnknownQueryVariable(varName, file, usageFunction): any {
  return report.warn(
...
async function parseToAst(
  ...
  fileStr: string,
  {
    ...
    addError?: (error: ParseError) => void | undefined;
  } = {},
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
): Promise<any> {
  let ast;
  ...
  if (transpiled && Array.isArray(transpiled)) {
...
function panicOnGlobalTag(file): IStructuredError | Array<IStructuredError> {
  return report.panicOnBuild(
...
function findApiExport(ast: string, api: string | null | undefined): boolean {
  let hasExport = false;
  const apiToFind = api ?? "";
...
async function findGraphQLTags(
  ...
  const documents: Array<GraphQLDocumentInFile> = [];
  ...
  function extractStaticQuery(taggedTemplateExpressPath, isHook = false): void {
    const {
      ast: gqlAst,
      text,
      hash,
    ...
    const docInFile: GraphQLDocumentInFile = {
      filePath: file,
      doc: gqlAst,
      text: text,
      hash: hash,
  ...
  function TaggedTemplateExpression(innerPath): void {
...
export default class FileParser {
  ...
  async parseFile(
    ...
    astDefinitions: Array<GraphQLDocumentInFile> | null;
    ...
    try {
      ...
      // default error
      let structuredError: ParseError = {
        id: "85915",
        context: {
          filePath: file,
        },
