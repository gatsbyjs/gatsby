...
import { IOptionalGraphQLInfoContext } from "gatsby-cli/src/structured-errors/types"
import { NameNode } from "graphql/language/ast"

interface IContext extends IOptionalGraphQLInfoContext {
...
const generateQueryName = ({
  ...
  if (!def.name || !def.name.value) {
    const slugified: string = slugify(file, {
      replacement: ` `,
      lower: false,
    })
    def.name = {
...
// there
function followVariableDeclarations(binding): Binding {
...
const warnForUnknownQueryVariable = (varName, file, usageFunction): void =>
  report.warn(
...
async function parseToAst(
  ...
  // error if we can't.
  const transpiled: string[] = await apiRunnerNode(`preprocessSource`, {
    filename: filePath,
    contents: fileStr,
...
interface IGraphQLDocumentInFile {
  filePath: string
  doc: DocumentNode
  templateLoc: string
  text: string
    ...
    parseToAst(file, text, { parentSpan, addError })
      .then(ast => {
        ...
        function TaggedTemplateExpression(
          innerPath
        ): Array<IGraphQLDocumentInFile> | void {
...
export class FileParser {
  ...
  async parseFile(file: string, addError): Promise<Array<DocumentNode> | null> {
    let text
    try {
    ...
    try {
      ...
      // default error
      let structuredError: IStructuredError = {
        id: `85915`,
        context: {
          filePath: file,
        },
  ...
  async parseFiles(
    ...
    const documents: Array<DocumentNode> = []
