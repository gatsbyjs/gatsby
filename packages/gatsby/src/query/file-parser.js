...
export default class FileParser {
  ...
  async parseFile(file: string, addError): Promise<?DocumentNode> {
    ...
    // We only process files that either include graphql, or static images, or SSR/DSG configs
    if (
    ...
    try {
      ...
      const astDefinitions =
        cache[hash] || (cache[hash] = await findGraphQLTags(ast, file))

      const isSSR = findApiExport(ast, `getServerData`)
      const hasConfig = findApiExport(ast, `config`) // This only checks for config existence, not if defer: true yet

      // TODO: Use page mode action to set SSR mode (and also DSG?)
