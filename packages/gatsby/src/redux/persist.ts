...
import { INode, ICachedReduxState } from "./types"
...
export function readFromCache(): ICachedReduxState {
  ...
  const nodes: [string, INode][] = [].concat(...chunks)
...
function guessSafeChunkSize(values: [string, INode][]): number {
...
function prepareCacheFolder(
  ...
  if (map) {
    ...
    const values: [string, INode][] = [...map.entries()]
