...
//   if (equal) {
//     return lte(prop1, prop2)
//   } else {
//     return lt(prop1, prop2)
//   }
// }
//
// function gtHelper(prop1, prop2, equal) {
//   if (equal) {
//     return gte(prop1, prop2)
//   } else {
//     return gt(prop1, prop2)
//   }
// }
...
function ltHelper(prop1, prop2, equal) {
  ...
  // handle array types
  if (Array.isArray(cv1) && Array.isArray(cv2)) {
    if (cv1.length === cv2.length) {
      for (let i = 0; i < cv1.length; i++) {
        const res = ltHelper(cv1[i], cv2[i], equal)
        if (res !== equal) {
          return res
        }
      }
...
function gtHelper(prop1, prop2, equal) {
  ...
  if (Array.isArray(cv1) && Array.isArray(cv2)) {
    ...
    if (cv1.length === cv2.length) {
      for (let i = 0; i < cv1.length; i++) {
        const res = gtHelper(cv1[i], cv2[i], equal)
        if (res !== equal) {
          return res
        }
      }
