import { difference, toPairs, get } from "lodash"
...
function getGatsbyUpgradeVersion(entries: IEntry[]): string {
...
function getErrorContext(
  ...
  const entries = badExports.map(function (ex) {
    return {
      ...ex,
...
export async function handleBadExports({ currentAPIs, badExports }) {
...
export function handleMultipleReplaceRenderers({ flattenedPlugins }) {
  // multiple replaceRenderers may cause problems at build time
  const rendererPlugins: string[] = flattenedPlugins
    .filter(function filter(plugin) {
      return plugin.ssrAPIs.includes(`replaceRenderer`)
    })
    .map(function mapper(plugin): string {
      return plugin.name
    })
  ...
  if (rendererPlugins.length > 1) {
    ...
    const ignorable: string[] = rendererPlugins.slice(0, -1)
    ...
    if (messages.length > 0) {
      ...
      messages.forEach(function callbackFn(m) {
        reporter.warn(m)
      })
...
interface IPackageJson {
  peerDependencies: { gatsby: string }
}
...
export function warnOnIncompatiblePeerDependency(
  ...
  const gatsbyPeerDependency: string = get(
    packageJSON,
