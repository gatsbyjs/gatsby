...
import { uniq, filter, flattenDeep } from "lodash"
...
import { extractQueries } from "../query/query-watcher"
import { writeAll } from "./requires-writer"
import { writeRedirects } from "./redirects-writer"
import { IPlugin } from "./load-plugins/load"

import { IBootstrapArgs, IModule } from "../types"

const tracer = opentracing.globalTracer()

// Show stack trace on unhandled promises.
process.on(`unhandledRejection`, function listener(reason): void {
  report.panic(reason)
})
...
export default async function (args: IBootstrapArgs) {
  ...
  emitter.on(`CREATE_JOB`, function handler(): void {
    if (!activityForJobs) {
  ...
  activity.start()
  const { configModule, configFilePath }: IModule = await getConfigFile(
    program.directory,
    `gatsby-config`
  )
  ...
  activity.start()
  const flattenedPlugins: IPlugin[] = await loadPlugins(
    config,
    program.directory
  )
  activity.end()
  ...
  const pluginsStr = uniq(
    flattenedPlugins.map(function mapper(p: IPlugin) {
  ...
  const pluginVersions = flattenedPlugins.map(function callbackfn(p: IPlugin) {
    return p.version
  })
  ...
  function hasAPIFile(env: string, plugin): string | void {
    // The plugin loader has disabled SSR APIs for this plugin. Usually due to
    // multiple implementations of an API that can only be implemented once
    if (env === `ssr` && plugin.skipSSR === true) {
      return undefined
    }
  ...
  const browserPlugins = filter(
    flattenedPlugins.map(function mapper(plugin: IPlugin) {
      return {
        resolve: hasAPIFile(`browser`, plugin),
        options: plugin.options,
      }
    }),
    function predicate(plugin: IPlugin) {
      return plugin.resolve
    }
  )

  const browserPluginsRequires = browserPlugins
    .map(function callbackfn(plugin: IPlugin): string {
      // we need a relative import path to keep contenthash the same if directory changes
      const relativePluginPath: string = path.relative(siteDir, plugin.resolve)
