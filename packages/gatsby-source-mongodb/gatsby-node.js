...
exports.sourceNodes = ({
  ...
  return mongoClient.connect().then(client => {
    ...
    return Promise.all(collection.map(col => createNodes(db, pluginOptions, dbName, createNode, createNodeId, col))).then(() => {
      mongoClient.close();
    }).catch(err => {
      console.warn(err);
      mongoClient.close();
        ...
        if (pluginOptions.map) {
          ...
          Object.keys(mapObj).forEach(mediaItemFieldKey => {
            if (node[mediaItemFieldKey] && (typeof mapObj[mediaItemFieldKey] === `string` || mapObj[mediaItemFieldKey] instanceof String)) {
              const mappingChildNode = prepareMappingChildNode(node, mediaItemFieldKey, node[mediaItemFieldKey], mapObj[mediaItemFieldKey], createNode);
