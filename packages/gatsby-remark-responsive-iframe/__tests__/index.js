...
const extractObjectTag = mdast => _.reduce(mdast.children[0].children, (result, child) => result + child.value, ``);
...
<${tag} url="http://www.example.com/" width="600" height="400"></${tag}>
    `);
      ...
      const _node$data$hChildren = node.data.hChildren,
...
<${tag} url="http://www.example.com/" width="600px" height="400px"></${tag}>
    `);
      ...
      expect(node.data.hChildren).toBeDefined();
      expect(node.data.hChildren.length).toBeGreaterThan(0);
      const _node$data$hChildren2 = node.data.hChildren,
            child = _node$data$hChildren2[0];
      expect(child.value).toMatchSnapshot();
    }));
  });
  const shouldntTransform = [[`100%`, `100`], [`100`, `100%`], [`invalid`, `100`], [`100`, `invalid`]];
