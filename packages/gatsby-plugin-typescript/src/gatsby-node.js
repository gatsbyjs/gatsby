...
export function modifyWebpackConfig(ctx) {
  ...
  const copts = Object.assign(compilerDefaults, compilerOptions, { module: 'commonjs' })
  ...
  config.loader('typescript', { test, loader: 'ts-loader?' + JSON.stringify(opts)})
}

export function preprocessSource(ctx) {
  const { args: { contents, filename }, pluginOptions: { compilerOptions } } = ctx
  // overwrite defaults with custom compiler options
  const copts = Object.assign(compilerDefaults, compilerOptions, { target: 'esnext', module: 'es6' })
