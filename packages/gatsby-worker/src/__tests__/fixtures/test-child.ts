...
export function sync(
  a: string,
  opts?: { addWorkerId?: boolean | undefined } | undefined,
): string {
  return `foo ${a}${
...
export function syncThrow(
  a: string,
  opts?:
    | { addWorkerId?: boolean | undefined; throwOnWorker?: number | undefined }
    | undefined,
): string {
  if (
    !opts?.throwOnWorker ||
...
type ILotOfMessagesTestMessage = {
...
let lotOfMessagesAndExit = function (_count: number): void {
  throw new Error("gatsby-worker messenger not available");
};
