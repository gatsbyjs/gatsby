...
const newPreviewAvailableClick = ({
  ...
  if (isOnPrettyUrl || window.location.hostname === 'localhost') {
    ...
    window.location.replace(`https://preview-${sitePrefix}${previewDomain && previewDomain[0]}${window.location.pathname}`);
  }
};
...
function GatsbyIndicatorButton({
  status,
  orgId,
  siteId,
  buildId,
