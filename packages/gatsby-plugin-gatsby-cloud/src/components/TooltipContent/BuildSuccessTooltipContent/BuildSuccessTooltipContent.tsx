...
const delay = (ms: number): Promise<void> => new Promise((resolve) => setTimeout(resolve, ms || 50));

const BuildSuccessTooltipContent: FC = () => {
  const { manifestInfo, buildInfo, isOnPrettyUrl, trackEvent } = useContext(IndicatorContext);
  ...
  const newPreviewAvailableClick = async (): Promise<void> => {
    ...
    if (manifestInfo?.redirectUrl) {
      window.location[isLocalhost ? `assign` : `replace`](manifestInfo?.redirectUrl);
