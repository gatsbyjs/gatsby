...
import { FEEDBACK_COOKIE_NAME, FEEDBACK_URL, INTERACTION_COOKIE_NAME } from "../../../constants";
...
const InfoIndicatorButton: FC = () => {
  ...
  const [buttonProps, setButtonProps] = useState<IIndicatorButtonProps>(baseButtonProps);
  ...
  useEffect(() => {
    ...
    const dispatcher: { [key: string]: () => void } = {
        ...
        if (showFeedback) {
          ...
          if (infoButtonProps.tooltip) {
            ...
            infoButtonProps.tooltip.content = (
              <FeedbackTooltipContent url={FEEDBACK_URL} onURLOpened={closeFeedbackTooltip} />
            );
      ...
      [BuildStatus.BUILDING]: (): void => prepareForMessage(`Building your preview...`),
      [BuildStatus.QUEUED]: (): void => prepareForMessage(`Kicking off your build...`),
    ...
    if (!manifestInfo?.errorMessage && !usingContentSync && currentBuildStatus) {
  ...
  return (
    <IndicatorButton {...buttonProps} onMouseEnter={!buttonProps?.disabled ? trackHoverEvent : undefined} />
  );
};
