...
const flotiqRequest = async (url, method, apiKey, contentTypeName, contentObject) => {
    let headers = {
...
const update = async ({ apiKey }) => {
  if (!apiKey) {
    console.error(
      `You must add the BENCHMARK_FLOTIQ_API_TOKEN env variable`
    )
    return
  }
  ...
  console.log("Updated article ID=" + article.id + ", new title: " + article.title)
