...
module.exports = function (pages, pagesReq) {
  ...
  const templateFiles = filter(pages, function (page) {
  ...
  for (let i = 0; i < templateFiles.length; i++) {
    ...
    if (!parentRoute) { parentRoute = templates.root }
  ...
  for (let j = 0, page; j < filteredPages.length; j++) {
    ...
    parentRoutes = filter(templateFiles, function () {
