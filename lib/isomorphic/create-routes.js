...
module.exports = (files, pagesReq) => {
  ...
  let rootTemplate
  try {
    rootTemplate = pagesReq('./_template')
  } catch (error) {
    if (includes(error.message, 'Cannot find module')) {
      rootTemplate = require('./defaults/_template')
      // FIXME link to a tutorial/guide/docs/something
      const defaultTemplateMessage = `
      Gatsby is currently using the default _template. You can override it by
      creating a React component at "/pages/_template.js".
      `
      console.info(defaultTemplateMessage)
    } else {
      throw error
    }
  }
