...
const main = (directory) => {
  let config = require(`${directory}/gatsby.config`)
  if (config.default) {
      ...
      const siteType = new GraphQLObjectType({
        ...
        fields: {
          ...
          allPages: {
            ...
            resolve (site, args) {
              let pages = {...routesDB.data}
              pages = _.values(pages)
              //console.log(routesDB)
              //console.log(pages)
              pages = pages.map((page) => { page.id = page.path; return page })
              const result = connectionFromArray(
                routesDB.data,
                args
              )
              result.totalCount = routesDB.data.length
              return result
            }
          }
        },
