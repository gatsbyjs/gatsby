...
module.exports = function (directory, callback) {
  // Read in site config.
  try {
    // const siteConfig = toml.parse(fs.readFileSync(directory + '/config.toml'))
  } catch (error) {
    // Do nothing
  }
  ...
  return glob(directory + '/pages/**/?(*.jsx|*.js|*.md|*.html)', null, (err, pages) => {
    if (err) { return callback(err) }

    for (let i = 0, page; i < pages.length; i++) {
      page = pages[i]
      let parsed
      let ext
      let data
      ...
      // const relativePath = path.relative(directory, page)
