...
module.exports = (program, directory, suppliedStage, webpackPort = 1500, pages = []) => {
    ...
    switch (stage) {
      case 'develop':
        return {
          commons: [
            ...
            `${directory}/.intermediate-build/app`,
          ],
        }
      case 'build-css':
        return {
          main: `${directory}/.intermediate-build/app`,
        }
      case 'build-html':
        return {
      ...
      case 'build-javascript':
        return {
          ...
          routes: `${directory}/.intermediate-build/production-app`,
  ...
  config.merge({
    ...
    entry: entry(),
    debug: true,
    profile: stage === 'production' ? true: false,
    devtool: devtool(),
    output: output(),
    resolveLoader: {
