...
import { site } from '../utils/globals'
...
module.exports = (program, directory, suppliedStage, webpackPort = 1500, routes = []) => {
  ...
  function entry () {
    switch (stage) {
      case 'develop':
        return {
          commons: [
            ...
            `${directory}/.intermediate-build/routes`,
          ],
        }
      case 'build-css':
        return {
          main: `${directory}/.intermediate-build/routes`,
        }
      case 'build-html':
        return {
      ...
      case 'build-javascript':
        return {
          //vendor: ["react", "react-dom", "react-router", "typography", 'react-responsive-grid', 'react-document-title'],
          //index: [`${directory}/pages/index`],
          //blogPost: [`${directory}/pages/blog-post`],
          //tagPage: [`${directory}/pages/tag-page`],
          //tagsPage: [`${directory}/pages/tags-page`],
          routes: `${directory}/.intermediate-build/split-routes`,
          // TODO create new chunks per *route* to soak up libs specific to each route
          // so each page chunk restricted to just data needed for that page.
          // Then "vendor" tweaking won't be needed as much.
          // TODO inline JS files < 10 kb?
        }
      default:
        throw new Error(`The state requested ${stage} doesn't exist.`)
    }
      ...
      case 'develop':
        return [
          ...
          new webpack.DefinePlugin({
            ...
            __LINK_PREFIX__: JSON.stringify(site().config.linkPrefix),
          }),
        ]
      case 'build-css':
        return [
