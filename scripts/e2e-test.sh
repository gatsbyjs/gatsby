...
cd "$SRC_PATH" &&
eval node "$GATSBY_PATH/packages/gatsby-dev-cli/dist/index.js" --set-path-to-repo "$GATSBY_PATH" &&
eval node "$GATSBY_PATH/packages/gatsby-dev-cli/dist/index.js" --force-install --scan-once  && # install _all_ files in gatsby/packages
